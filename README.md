...
| [`implement-review-comments`](./skills/engineer/implement-review-comments/SKILL.md) | Working through review comments — your own notes on the code, a PR's unresolved threads, or something you paste: sorts them by whether the implementation is obvious, applies that list on a single OK, grills you one at a time on the rest, and closes each one (deleted note, answered thread) the moment it lands. |
