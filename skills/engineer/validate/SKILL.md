...
Do the review **here, in this conversation** — every step below, yourself.

## Fresh eyes

Judge the code on its own merits against the spec. **The diff is the only
evidence.** A reviewer who leans on how the ticket got built inherits the
implementer's assumptions — the exact bias this review exists to remove. So even
in the session that wrote the code, and even where you remember why a line reads
the way it does, that line has to defend itself in the diff: no author's report
or commit message to lean on, and none to go looking for. Review the code, not
its author's intent.
...
- **Improve quality** against the code-standards bar: reduce nesting,
...
Close with the report the human reads, in their configured language. Cover:
**what you changed and why** (fixes, hardening, refactors); **what you flagged
but did NOT touch** (spec-coverage gaps, scope creep, judgment calls left to the
human); **tests you added** and what they protect against; and the
**typecheck/test status** (green, or exactly what's still failing).
