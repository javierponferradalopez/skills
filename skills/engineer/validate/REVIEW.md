...
You are reviewing one issue's uncommitted implementation with **fresh eyes** —
you did not write this code and hold no record of how it was built. Judge the
code on its own merits against the spec: there's no author's report or commit
message to lean on, and you shouldn't seek one. Review the code, not its
author's intent.
...
status** (green, or exactly what's still failing).
