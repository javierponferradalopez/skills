---
name: commit
description: Split a dirty working tree into an ordered list of atomic conventional commits, then commit them only on an explicit literal OK. Reads `git diff HEAD` to plan; messages are single-line.
