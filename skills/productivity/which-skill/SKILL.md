...
- **`/resolve-conflicts`**: a rebase, merge or cherry-pick stopped on conflict markers. It decides each conflict by the **intent** of the two sides and cites the evidence that proves the resolution, hunts the **silent breaks** git never marks, and moves git state only on an explicit OK. It also orchestrates a **stacked rebase**, where the cut point must be chosen before the first command.
